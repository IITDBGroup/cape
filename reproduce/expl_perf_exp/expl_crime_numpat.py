...
for test_id in range(1, 7):
	sct_df_prune = pd.read_csv('./time_record/crime_pruning_top3_exp_{}.csv'.format(str(test_id)),		
		names=['#attr', 'time'])
	sct_df_no_prune = pd.read_csv('./time_record/crime_no_pruning_top3_exp_{}.csv'.format(str(test_id)),		
		names=['#attr', 'time'])
