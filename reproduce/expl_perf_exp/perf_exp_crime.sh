...
do
    for topk in (3)
    do
        echo "Find top-${topk} explanations for Chicago crime data with ${pattern_size_dblp[$topk]} patterns using ExplGen-Naive"
...
do
    for topk in (3)
    do
        echo "Find top-${topk} explanations for Chicago crime data with ${pattern_size_dblp[$topk]} patterns using ExplGen-Opt"
        capexplain explain -u antiprov -d antiprov -p antiprov -P 5436 \
            --ptable dev.crime_subset --qtable crime_subset --ufile ./input/crime_${exp_id}.txt --ofile output.txt \
            --exp_id $exp_id --expl_topk=$topk --prune
    done
done
