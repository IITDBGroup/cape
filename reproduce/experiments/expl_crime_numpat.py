...
for test_id in range(1, 7):
	sct_df_prune = pd.read_csv('./expl_time_record/crime_pruning_top3_exp_{}.csv'.format(str(test_id)),		
		names=['#attr', 'time'])
	sct_df_no_prune = pd.read_csv('./expl_time_record/crime_no_pruning_top3_exp_{}.csv'.format(str(test_id)),		
		names=['#attr', 'time'])
...
sct_df = pd.DataFrame({'#patterns':test_id_list,'Prune':prune_time_list, 'No Prune':no_prune_time_list})
...
ax=tot.plot(x='#patterns',y='Prune',color='red',ax=ax,marker=mymarker[1],lw=mymarkerlw)
...
ax.legend(prop={'size': 30},labels=['ExplGen-Naive', 'ExplGen-Opt'],loc=2,
